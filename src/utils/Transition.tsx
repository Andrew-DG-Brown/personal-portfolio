...
export const TransitionAnimation = ({ isTransitioning }: { isTransitioning: boolean }) => {
    const [scope, animate] = useAnimate()

    const logoAnimation = async () => {
        await animate('svg', { opacity: 1, scale: 1, rotate: 20 }, { duration: 0.4, ease: 'easeOut'})
        await animate('svg', { scale: 0, rotate: 0, opacity: 0 }, { duration: 1, ease: 'anticipate'})
    }
    
    useEffect(() => {
        if (isTransitioning) logoAnimation()
    }, [isTransitioning])
